...
uniform Clock clocks[NUM_CLOCKS];
uniform int ClockCount;
...
// Currently draws a clock with fixed time
float clockSDF(vec3 pos, int i)
{
    vec3 v = clocks[i].seconds * (pos - clocks[i].position);
    float rim = torusSDF(v, clocks[i].dimensions.xy);
    float second = cylinderSDF(v - vec3(clocks[i].dimensions.x, 0, 0), clocks[i].dimensions.xz);
    float tenth = cylinderSDF(clocks[i].tenths * v 
        - vec3(clocks[i].dimensions.x, 0, 0), clocks[i].dimensions.xw);
    return unionSDF(rim, unionSDF(second, tenth));
